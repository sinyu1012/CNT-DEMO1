    ...
    class PrintDGV
    {
                    ...
                    System.Drawing.Printing.PrintEventArgs e)
        {
            try
            {
                ...
                // 计算总宽度
                //TotalWidth = 0;
                TotalWidth = -220;//如果调整了界面显示宽度，打印页面右边距在这里设置
                    ...
                    System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int tmpLeft = 30;//页面的左边距
            
            try
            {
                // 从第一页开始,之前保存标题和CoulmnType的宽度和高度
                if (PageNo == 1)
                {
                    foreach (DataGridViewColumn GridCol in dgv.Columns)
                    {
                ...
                while (RowPos <= dgv.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        ...
                        foreach (DataGridViewCell Cel in GridRow.Cells)
                        {
                            ...
                            {
                                CellCheckBox.Size = new Size(0, 0);
                            ...
                            else if (((Type)ColumnTypes[i]).Name == "DataGridViewImageColumn")
                            {
                                ...
                                e.Graphics.DrawImage((Image)Cel.FormattedValue,
                                        new Rectangle((int)ColumnLefts[i] + (int)((CelSize.Width - ImgSize.Width) / 2)-50,//X
                                        tmpTop + (int)((CelSize.Height - ImgSize.Height) / 2),//Y
                                        ((Image)(Cel.FormattedValue)).Width+100,//宽
                                        ((Image)(Cel.FormattedValue)).Height));//高
